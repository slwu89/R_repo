...
public:
  ...
  // constructor
  node() {};
  node(int initIx, char initState): ix(initIx), stateHistory(1,initState) {};
  
  // node attributes
  int ix; // node index
  bool infected = false; // is this node infected?
  double tInf = 0.0; // when this node was infected
  double mu = 0.0; //
  std::vector<char> stateHistory; // node state history
  std::vector<double> eventHistory; // node event history
  std::vector<int> infByHistory; // who infected this node
  
  // get attributes as an R list
  List getAttributes(){
    return(List::create(
        _["ix"] = ix,
        _["stateHistory"] = stateHistory,
        _["eventHistory"] = eventHistory,
        _["infByHistory"] = infByHistory
    ));
  }
